...
3. Когда увидите, кого оно задевает, переведите в `active`: исправьте
   `mode` в `rules.json` и проверьте файл командой `antibot rules check`.
   Нода перечитает его сама, без перезапуска.
